...
text_to_plot=data.frame(x=c(8.5,24.5,40.5,56.5),
                        ...
                        text=c(Reduce('paste0', c("Am\n",sumA,"\n",propA)),
                               ...
                               Reduce('paste0', c("Cm\n",sumC,"\n",propC)),
                               Reduce('paste0', c("Um\n",sumU,"\n",propU))))
line_to_plot1=data.frame(x=c(0,16.5),y=c(max(data$V2)+11,max(data$V2)+11))
...
line_to_plot4=data.frame(x=c(48.5,64.5),y=c(max(data$V2)+11,max(data$V2)+11))
p <- ggplot(data, aes(x=V4, y=V2))
p + geom_bar(stat="identity", position="dodge", aes(fill=V3), width = .7, show.legend = F, colour = 'black', size = 0.04669) +
  ...
  xlab("") +
  ylab("") +
  geom_text(aes(label=V2, y=V2+0.5), vjust=0, size=1) +
  geom_line(data = line_to_plot1,aes(x=x,y=y), size = 0.701, color = '#109648', show.legend = F) +
  geom_line(data = line_to_plot2,aes(x=x,y=y), size = 0.701, color = '#f7b32b', show.legend = F) +
  geom_line(data = line_to_plot3,aes(x=x,y=y), size = 0.701, color = '#255c99', show.legend = F) +
  geom_line(data = line_to_plot4,aes(x=x,y=y), size = 0.701, color = '#d62839', show.legend = F) +
  geom_text(data=text_to_plot,aes(x=x,y=y,label=text,fontface = "bold", vjust = 0.5), size = 1, show.legend = F) +
  theme(plot.margin = unit(c(0.4,0.5,-0.5,-0.5),'lines'),
        axis.text.x = element_text(angle = 90, vjust = 0.5, size = 4, face = "bold", colour = "#000000"),
        axis.text.y = element_text(face = "plain", colour = "#000000", size = 5),
...
ggsave(result_save_path, device = "pdf", width = 3.15, height = 1.1, dpi = "retina")
dev.off()