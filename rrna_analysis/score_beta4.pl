...
sub SCORE {
    ...
    for my $CURRENT ( 2 .. $#{$TR_END_COUNT} - 2 ) {
        ...
        my ( $SCORE1, $SCORE2, $SCORE3, $SCORE4, $SCORE5, $SCORE_LIMIT, $SCORE );
        ...
        $SCORE2 = $SCORE_LIMIT if ( $SCORE2 > $SCORE_LIMIT );
        $SCORE3 =
            ( $N_END_M2 * $N_END_M1 * $N_END_P1 * $N_END_P2 )**0.25 / $N_END;
        $SCORE3 = $SCORE_LIMIT if ( $SCORE3 > $SCORE_LIMIT );
        $SCORE4 = ( $N_START * $N_START_P2 )**0.5 / $N_START_P1;
        $SCORE4 = $SCORE_LIMIT if ( $SCORE4 > $SCORE_LIMIT );
